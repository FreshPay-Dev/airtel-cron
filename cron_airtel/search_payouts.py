...
query = f"SELECT source_account_number, trans_ref_no, partID, callback, merchant_ref, cycle_count FROM transactions WHERE DATE(created_at) = DATE(SUBDATE(NOW(), 1)) and financial_institution = 'airtel' and (`status` = 'Pending' or `status` = 'Submitted') and trans_type = 'payout'"
