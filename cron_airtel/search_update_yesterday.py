...
query = f"SELECT source_account_number, trans_ref_no, partID, callback, merchant_ref, cycle_count FROM transactions WHERE created_at BETWEEN CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY), '%Y-%m-%d'), ' 00:00:00') and CONCAT(DATE_FORMAT(DATE_SUB(CURRENT_DATE,INTERVAL 1 DAY), '%Y-%m-%d'), ' 23:59:59') and financial_institution = 'airtel' and status in ('Pending','Submitted') and trans_type = 'charge' and cycle_count <= 6 order by created_at DESC"
...
if ta > 0:
    for transaction in transactions:
        ...
        generatedAirtelVerif = subprocess.call('sudo php /var/www/html/airtel-cron-cluster/verify.php {}'.format(transaction_id),shell=True)
        generatedAirtelVerif = subprocess.call('sudo php /var/www/html/airtel-cron-cluster/run.php {}'.format(transaction_id),shell=True)
        try:
