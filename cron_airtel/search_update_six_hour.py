...
query = f"SELECT source_account_number, trans_ref_no, partID, callback, merchant_ref, cycle_count FROM transactions WHERE DATE(created_at) = DATE(SUBDATE(NOW(), 0)) and financial_institution = 'airtel' and `status` = 'Pending' and trans_type = 'charge' and (cycle_count >= 26 and cycle_count <= 29)"
