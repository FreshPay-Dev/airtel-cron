...
query = f"SELECT source_account_number, trans_ref_no, partID, callback, merchant_ref FROM transactions WHERE DATE(created_at) = DATE(SUBDATE(NOW(), 0)) and financial_institution = 'airtel' and `status` = 'Pending' and trans_type = 'charge'"
    ...
    for transaction in transactions:
        ...
        try:
            ...
            if FinancialInstitutionID != "null":
                ...
                query = "UPDATE transactions SET updated_at = %s, status = %s, financial_institution_transaction_id = %s, financial_institution_status_description = %s WHERE trans_ref_no = %s"
                dataToInsert = (updatedAt, status, FinancialInstitutionID, statusDesc, transaction_id)
                ...
                conn = connectToDatabase(host='167.172.169.188', user='jbiola', password='gofreshbakeryproduction2020jb', db='switch', port=3306)
                updatedAt = str(datetime.now())
                query = "UPDATE transactions SET updated_at = %s, status = %s, financial_institution_status_description = %s  WHERE trans_ref_no = %s"
                dataToInsert = (updatedAt, status, statusDesc, transaction_id)
                updatedToSwitch = executeQueryForInsertDate(conn, query, dataToInsert)    
